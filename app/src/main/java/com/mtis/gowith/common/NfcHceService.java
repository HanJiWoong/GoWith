...
public class NfcHceService extends HostApduService {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent.hasExtra("memberId"))
            mMemberId = intent.getExtras().getInt("memberId");
        else
            mMemberId = 0;
        Log.e(TAG, "onStartCommand, mMemberId= " + mMemberId);
    ...
    public byte[] processCommandApdu(byte[] commandApdu, Bundle extras) {
        Log.e(TAG, "Received APDU: " + ByteArrayToHexString(commandApdu) + ", " + ByteArrayToHexString(SELECT_APDU));

        // If the APDU matches the SELECT AID command for this service,
        // send the loyalty card account number, followed by a SELECT_OK status trailer (0x9000).
        if (Arrays.equals(SELECT_APDU, commandApdu)) {
            Log.e(TAG, "SELECT_OK_SW");
//            return SELECT_OK_SW;
            return HexStringToByteArray("9000" + String.format("%04X", mMemberId));
        } else {
